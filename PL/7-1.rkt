  ...
  (type-case ExprC a
    ...
    [appC (name arg)
          (let* ((arg-val (interp arg env))
                 (func (interp name env)) ;should be funcV
                 (func-arg (funcV-arg func))
                 (func-body (funcV-body func)))
            (interp func-body
                    (hash-set (hash empty) func-arg arg-val)))]))
;;; test
(parse '(do-something plus23))
(desugar (parse '(do-something plus23)))
(interp (desugar (parse '(do-something-to-1 plus23))) 
          (hash 
           (list (values 'do-something-to-1 (funcV 'x (appC (idC 'x) (numC 1))))
                 (values 'plus23 (funcV 'x (plusC (numC 23) (idC 'x)))))))

;;; test broken
(interp (appC
         (appC
          (funcC 'x (funcC 'y (plusC (idC 'y) (idC 'x))))
          (numC 2))
         (numC 3))
        (hash empty))