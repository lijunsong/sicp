...
(define (interp [a : ExprC] [env : Env]) : CVal
  (type-case ExprC a
    ...
    [boxC (arg)
          (boxV (interp arg env))]
    [unboxC (arg)
            (let ((v-arg (interp arg env)))
              (if (not (boxV? v-arg))
                  (error 'interp "unbox a none-box")
                  (boxV-v v-arg)))]
    [setboxC (b v)
             (let ((v-v (interp v env))
                   (v-b (interp b env)))
               (if (not (boxV? v-b))
                   (error 'interp "setbox to a none-box-type expr")
                   (begin ;set v-b to v-v, but how?
                          v-b)))]
    [seqC (b1 b2) ; actually begin can be used here, since we just computs value and won't store anything
